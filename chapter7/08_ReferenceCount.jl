...
mutable struct MazeStateByBitSet
    ...
    function MazeStateByBitSet(state::WallMazeState)
        ...
        new(whole_point_mat, walls, state, 1)
    end
end

function add_ref!(state::Base.RefValue{WallMazeState})
    getindex(state).ref_count += 1
end

function del_ref!(state::Base.RefValue{WallMazeState})
    getindex(state).ref_count -= 1
end



mutable struct StatePtr
    state::Base.RefValue{WallMazeState}
    ref_count::Int64
    function StatePtr(state::Base.RefValue{WallMazeState})
        add_ref!(state)
        new(state, 1)
    end

    function StatePtr(ptr::StatePtr)
        add_ref!(ptr.state)
        new(ptr.state, ptr.ref_count)
    end

end

function Base.isless(state_ptr1::StatePtr, state_ptr2::StatePtr)::Bool
    state_ptr1.state < state_ptr2.state
end

function Base.isless(state1::Base.RefValue{WallMazeState}, state2::Base.RefValue{WallMazeState})::Bool
    getindex(state1).evaluated_score < getindex(state2).evaluated_score
end
...
function beam_search_action(state::WallMazeState, beam_width::Int, beam_depth::Int)::Int64
    now_beam = PriorityQueue{StatePtr, StatePtr}(Base.Order.Reverse)
    enqueue!(now_beam, StatePtr(Base.RefValue(state)), StatePtr(Base.RefValue(state)))
    best_state = state
    hash_check = Set(Int[])
    for t in 1:beam_depth
        next_beam = PriorityQueue{StatePtr, StatePtr}(Base.Order.Reverse)
        for i in beam_width
            if isempty(now_beam)
                break
            end
            now_state = getindex(dequeue!(now_beam).state)
            ...
            for action in actions
                ...
                enqueue!(next_beam, StatePtr(Base.RefValue(next_state)), StatePtr(Base.RefValue(next_state)))
            end
        end
        
        now_beam = next_beam
        if !isempty(now_beam)
            best_state = getindex(first(now_beam)[1].state)
        end
